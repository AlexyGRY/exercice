...
console.log(data);
const quoteAleatoire=document.getElementById('aleatoire')
...
function generationQuote() {
    ...
    } while (randomQuote === quoteInitial);
    quoteInitial= randomQuote;
    console.log(data[quoteInitial]);
    
}

const generer=document.getElementById('generer')
function nextQuote() {
    quoteInitial = (quoteInitial + 1) % data.length;
    console.log(data[quoteInitial]);
    
}
quoteAleatoire.addEventListener('click',generationQuote);
generer.addEventListener('click',nextQuote)
